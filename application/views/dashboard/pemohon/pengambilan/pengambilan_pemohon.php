<div class="row">

    <div class="col-md-6">
        <div class="box">
        <div class="box-header with-border">
            <h3 class="box-title">Ketentuan Pengambilan Ijazah dan Transkrip Asli</h3>

            <div class="box-tools pull-right">
            <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i>
            </button>
            
            <button type="button" class="btn btn-box-tool" data-widget="remove"><i class="fa fa-times"></i></button>
            </div>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
        1. Pengambilan ijazah dan transkrip asli hanya bisa dilakukan dengan cara diambil di kampus Politeknik Statistika STIS langsung oleh pemohon atau oleh orang lain yang telah diberi kuasa. 
        </br>2. Untuk alumni yang telah menyelesaikan masa Ikatan Dinas (ID):
        </br>a. <b>Surat Keterangan (SK) masih bekerja di BPS</b> yang mengetahui (tanda tangan) minimal eselon III
        </br>b. Surat Kuasa bila pengambilan dikuasakan/diwakilkan
        </br>3. Untuk alumni yang belum menyelesaikan masa Ikatan Dinas (ID):
        </br>a. <b>Bukti Asli pembayaran tuntutan ganti rugi (TGR) dari Kementerian Keuangan</b>   
        </br>b. <b>Materai Rp.6000,-</b>
        </br>Surat Kuasa bila pengambilan dikuasakan/diwakilkan
        </br>4. Pemohon memilih maksimal 5 hari kerja sebelum tanggal pengambilan misalnya, pemohon yang akan mengambil ijazah asli di tanggal 10 maka harus mengkonfirmasi paling lambat tanggal 5.
        </br>5. Pemohon yang telah menerima dokumen harus mengkonfirmasi penerimaan dokumen agar dapat mengajukan permohonan lainnya di kemudian hari.

        </div>

        <div class="box-footer clearfix">
            <a href="<?php echo base_url('pemohon/pengambilan/pengajuan')?>"><button type="button" class="pull-right btn btn-default" >Ajukan Permohonan
            <i class="fa fa-arrow-circle-right"></i></button></a>
        </div>

        <!-- ./box-body -->
        </div>
        <!-- /.box -->
    </div>
    <!-- /.col -->

    <div class="col-md-6">

        <div class="box-body">
            <a href="" class="btn btn-lg btn-primary btn-block" style="margin-bottom: 10px">Surat Keterangan</a>
            <a href="" class="btn btn-lg btn-primary btn-block" style="margin-bottom: 10px">Bukti Asli Pembayaran TGR</a>
            <a href="" class="btn btn-lg btn-primary btn-block" style="margin-bottom: 10px">Surat Kuasa</a>
        </div>
        <!-- ./box-body -->

    </div>
    <!-- /.col -->

</div>